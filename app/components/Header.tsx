...
export default function Header() {
  ...
  return (
    // NOTE: backdrop-blur removed from here — a filter/backdrop-filter on this
    // element would make it the containing block for its `fixed` children
    // (the overlay + drawer below), breaking their positioning/click area.
    <nav className="fixed top-0 w-full z-50 border-b border-border-dark shadow-lg shadow-electric-blue/5 transition-all duration-300">

      {/* Top bar — blur lives here instead, isolated from the fixed overlay/drawer */}
      <div className="bg-background/90 backdrop-blur-md">
        <div className="flex md:grid md:grid-cols-3 items-center justify-between md:justify-normal max-w-[1550px] mx-auto px-4 sm:px-6 lg:px-8 py-4">
      ...
      {/* Overlay — now a direct fixed child of <nav>, which has no filter,
          so this correctly covers the full viewport and receives clicks anywhere */}
      {menuOpen && (
        <div
      ...
      {/* Mobile Right-Side Drawer — solid background, no reliance on a
          possibly-transparent custom "background" token */}
      <div
        className={`md:hidden fixed top-0 right-0 h-full w-[280px] z-50 bg-obsidian-black border-l border-border-dark transition-transform duration-300 ease-in-out ${menuOpen ? "translate-x-0" : "translate-x-full"}`}
      >
